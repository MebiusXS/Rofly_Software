...
void GridMap::initMap(ros::NodeHandle &nh)
{
  ...
  occ_timer_ = node_.createTimer(ros::Duration(0.048), &GridMap::updateOccupancyCallback, this);
...
void GridMap::raycastProcess(const pcl::PointCloud<pcl::PointXYZ> &cloud)
{
  ...
  while (!md_.cache_voxel_.empty())
  {
    ...
    if (md_.occupancy_buffer_[idx_ctns] < mp_.clamp_min_log_ - 1e-3)
