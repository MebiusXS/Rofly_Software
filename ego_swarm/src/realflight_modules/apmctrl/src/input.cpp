...
void RC_Data_t::feed(mavros_msgs::RCInConstPtr pMsg)
{
    ...
    mode = ((double)msg.channels[4] - 1000.0) / 1000.0; // Ch5, SE mode.
    // gear = ((double)msg.channels[5] - 1000.0) / 1000.0; // Ch6, SF trigger.
    gear = (1945.0 - 1000.0) / 1000.0; // Delete double check(conflict with goal confirmation).
    reboot_cmd = ((double)msg.channels[15] - 1000.0) / 1000.0; // Ch15, LD2, reboot.
...
// #define VEL_IN_BODY
    ...
    if ((now - t_last).toSec() > 0.2)
    {
        ROS_INFO("ODOM: (now - t_last).toSec() = %f", (now - t_last).toSec());
    }
    t_last = now;
}
...
void Imu_Data_t::feed(sensor_msgs::ImuConstPtr pMsg)
{
    ...
    static ros::Time t_last(0);
    if ((now - t_last).toSec() > 0.1)
    {
        ROS_INFO("IMU: (now - t_last).toSec() = %f", (now - t_last).toSec());
    }
    t_last = now;

    quadrotor_msgs::Px4ctrlDebug imu_recv_msg; //debug
    tic_pub.publish(imu_recv_msg);
...
void Command_Data_t::feed(quadrotor_msgs::PositionCommandConstPtr pMsg)
{
    ...
    // Debug:
    // ROS_INFO("\033[34mRECEIVE COMMAND DATA!\033[0m");
}
