...
void PX4CtrlFSM::process()
{
	...
	case MANUAL_CTRL:
	{
		check_nonstatic_vel_on_ground(odom_data, AutoTakeoffLand_t::ARM_ALLOWED_VEL);
		...
		else if (param.takeoff_land.enable && takeoff_land_data.triggered && takeoff_land_data.takeoff_land_cmd == quadrotor_msgs::TakeoffLand::TAKEOFF) // Try to jump to AUTO_TAKEOFF
		{
			...
			if (odom_data.v.norm() > AutoTakeoffLand_t::ARM_ALLOWED_VEL)
			{
	...
	case AUTO_HOVER:
	{
		// ROS_WARN("cmd_is_received: %d, is_cmd_mode: %d",cmd_is_received(now_time), rc_data.is_command_mode);
		...
		{
			// ROS_WARN("[1111");
			// ROS_WARN("mode is %s",state_data.current_state.mode.c_str());
		...
		{
			// set_hov_with_rc(); // Drone can by controlled by rc while running ego.
			des = get_hover_des();
	...
	case CMD_CTRL:
	{
		...
		else if (!rc_data.is_command_mode || !cmd_is_received(now_time))
		{
			...
			// Modified:
			ros::param::set("/cmd2autohover", true); // Reset marker to drone after finishing ego_planner
		}
		else
		{
			des = get_cmd_des();
...
void PX4CtrlFSM::land_detector(const State_t state, const Desired_State_t &des, const Odom_Data_t &odom)
{
	...
	constexpr double VELOCITY_THR_C = 0.15;		  // Constraint 2: velocity below VELOCITY_MIN_C m/s.
...
{
	// ROS_INFO("\033[32mCommand is received!\033[32m");
...
bool PX4CtrlFSM::check_nonstatic_vel_on_ground(const Odom_Data_t &odom, const double allowd_vel)
{
	static ros::Time last_print_time(0);
	bool ret = false;

	// ROS_ERROR("A v=%f", odom.v.norm());
	if ( get_landed() && odom.v.norm() > allowd_vel )
	{
	// ROS_ERROR("AA");
		ros::Time now = ros::Time::now();
		if ( (now - last_print_time).toSec() > 1.0 )
		{
			ROS_WARN("Non-static velocity on ground: vel=%f", odom.v.norm());
			last_print_time = now;
		}
		ret = true;
	}
