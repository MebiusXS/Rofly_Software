# PX4 controller V1.3 先不要开源 除了最后一章“PX4 Controller详细介绍”以外都先看一遍免得踩坑

### 2021-8-5 更新：1. 修改了数据输入频率判断方法，避免数据不稳导致的误报； 2. 自动起飞后延迟2秒才发trigger，使得发trigger时飞机初速度尽量接近0

# 使用说明

- [PX4 controller V1.3 先不要开源 除了最后一章“PX4 Controller详细介绍”以外都先看一遍免得踩坑](#px4-controller-v13-先不要开源-除了最后一章px4-controller详细介绍以外都先看一遍免得踩坑)
    - [2021-8-5 更新：1. 修改了数据输入频率判断方法，避免数据不稳导致的误报； 2. 自动起飞后延迟2秒才发trigger，使得发trigger时飞机初速度尽量接近0](#2021-8-5-更新1-修改了数据输入频率判断方法避免数据不稳导致的误报-2-自动起飞后延迟2秒才发trigger使得发trigger时飞机初速度尽量接近0)
- [使用说明](#使用说明)
  - [飞控设置](#飞控设置)
...
**定点模式**: 终端打印绿色的"AUTO_HOVER(L2)"， 此时控制器会通过代码接管飞控，飞控会处于"GUIDED_NOGPS"模式.控制器依据odometry反馈做定点控制，此时拨动方向、油门等摇杆，飞机会慢慢移动，就像在飞大疆Mavic的定点模式一样，但此时不接收你的代码发出的指令.  
