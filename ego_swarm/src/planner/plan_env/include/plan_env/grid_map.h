...
struct MappingData
{
  ...
  Eigen::Vector3d camera_pos_, last_camera_pos_, body_pos_;
...
inline int GridMap::getInflateOccupancy(Eigen::Vector3d pos)
{
  ...
  if (abs(md_.body_pos_(0) - pos(0)) < 1.1 * mp_.resolution_ && 
      abs(md_.body_pos_(1) - pos(1)) < 1.1 * mp_.resolution_ && 
      abs(md_.body_pos_(2) - pos(2)) < 1.1 * mp_.resolution_) 
    return 0;
