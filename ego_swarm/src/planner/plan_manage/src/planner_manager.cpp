...
namespace ego_planner
{
  ...
  void EGOPlannerManager::initPlanModules(ros::NodeHandle &nh, PlanningVisualization::Ptr vis)
  {
    ...
    vel_acc_vec_[0] = pp_.max_vel_;
    vel_acc_vec_[1] = pp_.max_acc_;

    vel_acc_sub_ = nh.subscribe("planning/vel_acc_vec", 1, &EGOPlannerManager::VACallback, this);
    ...
    ploy_traj_opt_->set_vel_acc(vel_acc_vec_);
  ...
  void EGOPlannerManager::VACallback(const std_msgs::Float64MultiArray::ConstPtr& msg)
  {
    if(msg->data.size() < 2) return;
    vel_acc_vec_[0] = msg->data[0];
    vel_acc_vec_[1] = msg->data[1];
    ploy_traj_opt_->set_vel_acc(vel_acc_vec_);
    std::cout << "Callback executed: (" << vel_acc_vec_[0] << ", " << vel_acc_vec_[1] << ")" << std::endl;
  }
      ...
      const bool flag_randomPolyTraj, const bool touch_goal)
  {
    ...
    double ts = pp_.polyTraj_piece_length / vel_acc_vec_[0];

    std::cout << "reboundReplan max_vel = " << vel_acc_vec_[0] << std::endl;
      ...
      else
      {
        ...
        innerPs = (start_pt + local_target_pt) / 2 +
                  (((double)rand()) / RAND_MAX - 0.5) *
                      ...
                      horizen_dir * 0.4 * (-0.978 / (continous_failures_count_ + 0.989) + 0.989) +
                  (((double)rand()) / RAND_MAX - 0.5) *
                      (start_pt - local_target_pt).norm() *
                      vertical_dir * 0.2 * (-0.978 / (continous_failures_count_ + 0.989) + 0.989);
      ...
      Eigen::Vector3d &local_target_vel, bool &touch_goal)
  {
    ...
    double t_step = planning_horizen / 20 / vel_acc_vec_[0];
    std::cout << "getLocalTarget max_vel = " << vel_acc_vec_[0] << std::endl;
    ...
    if ((global_end_pt - local_target_pos).norm() < (vel_acc_vec_[0]* vel_acc_vec_[0]) / (2 * vel_acc_vec_[1]))
    {
    ...
    std::cout << "getLocalTarget max_acc = " << vel_acc_vec_[1] << std::endl;
  }
      ...
      const Eigen::Vector3d &end_vel, const Eigen::Vector3d &end_acc)
  {
    ...
    double des_vel = vel_acc_vec_[0] / 1.5;
    std::cout << "planGlobalTrajWaypoints max_vel = " << vel_acc_vec_[0] << std::endl;
    ...
    for (int j = 0; j < 2; ++j)
    {
      ...
      if (globalMJO.getTraj().getMaxVelRate() < vel_acc_vec_[0] ||
          start_vel.norm() > vel_acc_vec_[0] ||
          end_vel.norm() > vel_acc_vec_[0])
      {
        break;
      }
