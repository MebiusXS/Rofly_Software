...
namespace ego_planner
{
  ...
  void EGOReplanFSM::init(ros::NodeHandle &nh)
  {
    ...
    // Modified:
    int my_flight_type;
    if (ros::param::has("/my_flight_type")) {
      ros::param::get("/my_flight_type", my_flight_type);
      ros::param::set("/drone_0_ego_planner_node/fsm/flight_type", my_flight_type);
    }
    ...
    no_replan_thresh_ = 0.5 * emergency_time_ * planner_manager_->vel_acc_vec_[0];
    std::cout << "EGOReplanFSM::init max_vel = " << planner_manager_->vel_acc_vec_[0] << std::endl;
    ...
    replan_state_pub_ = nh.advertise<std_msgs::Bool>("/replan_state_flag", 10);
    // goal_achieved_pub_ = nh.advertise<std_msgs::Bool>("/goal_achieved_flag", 10);
    occurred_error_pub_ = nh.advertise<std_msgs::Bool>("/occurred_error_flag", 10);
    ...
    else if (target_type_ == TARGET_TYPE::PRESET_TARGET)
    {
      ...
      std::cout << "readGivenWpsAndPlan!!!" << std::endl;
      readGivenWpsAndPlan();
    }
    else
  ...
  void EGOReplanFSM::execFSMCallback(const ros::TimerEvent &e)
  {
    ...
    // std::cout << last_time.toNSec() << std::endl;
    // ROS_ERROR(" ");
    ...
    case SEQUENTIAL_START: // for swarm or single drone with drone_id = 0
    {
      if (planner_manager_->pp_.drone_id <= 0 || (planner_manager_->pp_.drone_id >= 1 && have_recv_pre_agent_))
      {
        ...
        /*modified by czk*/
        std_msgs::Bool plan_global_flag;
        plan_global_flag.data = success;
        replan_state_pub_.publish(plan_global_flag);
    ...
    case REPLAN_TRAJ:
    {
      ...
      /*modified by czk*/
      std_msgs::Bool plan_local_flag;

      if (planFromLocalTraj(1))
      {
        plan_local_flag.data = true;
        changeFSMExecState(EXEC_TRAJ, "FSM");
      }
      else
      {
        plan_local_flag.data = false;
        changeFSMExecState(REPLAN_TRAJ, "FSM");
      }

      replan_state_pub_.publish(plan_local_flag);
    ...
    case EXEC_TRAJ:
    {
      ...
      double t_fixed_frq = (ros::Time::now() - last_time).toSec();
      ...
      /*modified by czk*/
      // std_msgs::Bool goal_achieved_flag, occurred_error_flag;
      // goal_achieved_flag.data = touch_the_goal;
      // goal_achieved_pub_.publish(goal_achieved_flag);
      std_msgs::Bool occurred_error_flag;
      occurred_error_flag.data = false;

      if (mondifyInCollisionFinalGoal()) // case 1: find that current goal is in obstacles
      {
        occurred_error_flag.data = true;
        // pass
      }
      else if ((target_type_ == TARGET_TYPE::PRESET_TARGET) &&
    ...
    case EMERGENCY_STOP:
    {
      ...
      std_msgs::Bool occurred_error_flag;
      occurred_error_flag.data = true;
      occurred_error_pub_.publish(occurred_error_flag);
  ...
  bool EGOReplanFSM::mondifyInCollisionFinalGoal()
  {
    if (planner_manager_->grid_map_->getInflateOccupancy(final_goal_))
    {
      ...
      double t_step = planner_manager_->grid_map_->getResolution() / planner_manager_->vel_acc_vec_[0];
      std::cout << "EGOReplanFSM::mondifyInCollisionFinalGoal max_vel = " << planner_manager_->vel_acc_vec_[0] << std::endl;
  ...
  bool EGOReplanFSM::measureGroundHeight(double &height)
  {
    ...
    double forward_t = 2.0 / planner_manager_->vel_acc_vec_[0];
    std::cout << "EGOReplanFSM::measureGroundHeight max_vel = " << planner_manager_->vel_acc_vec_[0] << std::endl;
